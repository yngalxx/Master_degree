...
@click.option(
    ...
    type=float,
    help="Minimum confidence level of the predictions considered in calculating the mAP metric.",
    show_default=True,
)
@click.option(
    "--train_set",
...
def calculate_metric(main_dir, min_conf_level, train_set, test_set, val_set):
    ...
    if train_set:
        try:
            ...
            pred_list, ground_truth_list = prepare_data_for_ap(output_list=out, target_list=expected)

            map_dict = calculate_map(pred_list, ground_truth_list, 6, min_conf_level)
        ...
        logging.warning('Argument "train_set" not passed, calculating metric for this set is skipped')
    ...
    if val_set:
        ...
        logging.warning('Argument "val_set" not passed, calculating metric for this set is skipped')
    ...
    if test_set:
        ...
        logging.warning('Argument "test_set" not passed, calculating metric for this set is skipped')


    if not train_set and not val_set and not test_set:
        logging.error('None of the arguments: "train_set", "val_set" and "test_set" passed, aborted!')
        raise ValueError()
