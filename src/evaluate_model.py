...
def evaluate_model(
    ...
    device = torch.device(torch.cuda.current_device()) if cuda_statement and gpu else torch.device("cpu")
    ...
    if test:
        ...
        map_df = pd.DataFrame.from_dict(calculate_map(model_output, ground_truth, num_classes=num_classes), orient="index", columns=["AP"])
