...
@click.option(
    '--directory_path', 
    default="/".join(str(pathlib.Path(__file__).parent.resolve()).split("/")[:-1]), type=str, 
...
def image_scraper(directory_path):
    ''' 
    simple scraper to retriev full-resolution images from urls finded in COCO formatted
    files with annotations obtained from source repository (newspaper-navigator-master)
    '''
