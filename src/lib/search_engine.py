...
df_ocr = pd.read_sql_query("SELECT CLEANED_TEXT, PRED_LABEL, FILE_NAME FROM OCR_RESULTS", conn)

df_ocr['SEARCH_TEXT'] = df_ocr['PRED_LABEL'].str.lower() + ' ' + df_ocr['CLEANED_TEXT'].str.lower()

df_keywords = pd.read_sql_query("SELECT KEYWORD, FILE_NAME FROM KEYWORDS", conn)
...
grouped_keywords_list = [" ".join(df_keywords['KEYWORD'][df_keywords['FILE_NAME']==unique_file].values) for unique_file in tqdm(unique_files)]
...
schema = fields.Schema(filename=fields.TEXT(stored=True), cleantext=fields.TEXT(stored = True), keywords=fields.TEXT(stored=True))
 
ix = index.create_in(f"{main_dir}/{temp_index_dir}", schema)

writer = ix.writer()
for i in tqdm(range(len(df_ocr)), desc='Indexing documents'):
    writer.add_document(filename=df_ocr['FILE_NAME'][i], cleantext=df_ocr['SEARCH_TEXT'][i], keywords=df_ocr['KEYWORDS'][i])
writer.commit()

cleantext_list, file_name_list, keywords_list = [], [], []
with ix.searcher() as searcher:
     query = qparser.QueryParser("cleantext", ix.schema).parse("sausage".lower())
...
results_df['cleantext'] = [x.split(maxsplit=1)[1] for x in results_df['cleantext']]
